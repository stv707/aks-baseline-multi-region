...
1.  Get the corresponding AKS cluster spoke VNet resource IDs for the app team working on the application A0042.
    ...
    1.  Create the Azure Credentials for the GitHub CD workflow.
        ...
        #
        # Note that you will be running the next command using the --sdk-auth flag which is marked 
        # as deprecated; however, the github workflow's Azure login action, using the credentials 
        # generated by this command's output will still be successful. The Azure login action 
        # supports an improved way of authenticating with Azure using a Federated Identity 
        # Credential. We plan on including this new method in the next updates.
        #
